...
- ~~**Pre-existing security note**: a real-looking Gemini API key committed in `appsettings.Development.json`.~~ **RETRACTED 2026-08-15 — this was wrong.** `git ls-files` confirms no `appsettings.Development.json` is tracked; it is covered by `.gitignore` (`**/appsettings.Development.json`). The key exists only in the local working copy, which is exactly the mechanism `error-handling-and-config.md` prescribes for dev secrets. No rotation needed on this account.
