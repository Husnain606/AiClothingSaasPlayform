...
public class MeasureAuthenticationAcceptanceTests(WebApplicationFactory<Program> factory) : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task PostMeasure_NoToken_Returns401()
    {
        HttpClient client = factory.CreateClient();
        using MultipartFormDataContent content = new();
        HttpResponseMessage response = await client.PostAsync(new Uri("/api/measure", UriKind.Relative), content);
        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    [Fact]
    public async Task PostMeasure_ValidTokenSignedWithSharedSecret_PassesAuthentication()
    {
        ...
        // A missing multipart body fails model binding/validation (400), not authentication (401/403) —
